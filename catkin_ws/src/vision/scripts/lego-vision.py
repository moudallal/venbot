...
def rgb2name(rgb):
    ...
    for color, color_rgb in COLORS.items():
        distance = math.dist(rgb, color_rgb)
...
def process_item(imgs, item):
    ...
    msg = LegoStates()
    msg.name = nm
    msg.color = rgb2name(list(reversed(color)))
    #fov = 1.047198
    #rap = np.tan(fov)
    #print("rap: ", rap)
...
def process_image(rgb, depth):    
    ...
    for point in results:
        if point is not None:
            ...
            msg.color.append(point.color)
            print(point.color)
            # rospy.INFO(f'Brick detected! ID: {point.name}, Color: {str(point.color)}')
    pub.publish(msg)
...
def start_node():
    ...
    pub = rospy.Publisher("lego_detections", LegoStates, queue_size=1)
